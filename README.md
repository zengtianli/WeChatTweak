...
>
> `keeptip` 变体在 **build 269136（4.1.11）实机实测**：**私聊**撤回后消息保留且显示提示；**群聊**撤回消息虽保留、但仍无提示（表现同静默变体）。原因是清零 newmsgid 的做法在群聊里连带掐掉了提示渲染——群聊提示与「删哪条」耦合在同一下游路径，而该路径经虚派发分发、纯字节补丁定位不到。群聊要出提示需走运行时注入（见[「留提示」变体](#留提示变体--variant-keeptip)末尾）。issue [#1038](https://github.com/sunnyyoung/WeChatTweak/issues/1038) 中 wuliyc 报告过在 4.1.11 上的同类效果（未说明群/私聊范围）。
>
...
>
> **群聊为什么没提示**：整个 revoke 模块（`0x48a0140..0x48ad700`）里写 newmsgid 字段 `[x19,#0x168]` 的只有 `0x48a0b44` 一处，私聊群聊共用它。私聊的提示插入不依赖 newmsgid、照出；群聊的提示渲染依赖 newmsgid（决定挂在哪条下面），被我们清零后连带失效 → 群聊静默。这个下游消费者经虚派发/chained-fixup 分发，纯字节补丁静态定位不到独立的「群聊提示」点。**群聊要出提示只能走运行时注入**（注入 dylib hook 撤回处理、客户端自行合成并插入提示，即 fzlzjerry 的 `--runtime-tip` 路线），本 fork 暂未纳入。
