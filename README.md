...
| **防撤回（留提示变体）** | 消息留着 **且** 仍显示「对方撤回了一条消息」提示 | ✓ | ✓（`--variant keeptip`，269136 实测有提示） |
...
>
> `keeptip` 变体在 **build 269136（4.1.11）上已实机实测通过**：撤回后消息保留、且显示「对方撤回了一条消息」提示。issue [#1038](https://github.com/sunnyyoung/WeChatTweak/issues/1038) 中 wuliyc 报告过在 4.1.11 上的同类效果。其它 4.x 构建号若要用 `keeptip`，需先按[新增版本](#新增一个版本)定位 `0x…b44` 处 `str x0,[x19,#0x168]`（存 newmsgid）的偏移，加一条 `revoke-keeptip` 补丁点。
>
...
>
> **状态**：字节补丁已实现，并在 **build 269136（4.1.11）上实机实测通过**——撤回后消息保留、且显示「对方撤回了一条消息」提示。（静态复核：打补丁后 `0x48a03b0` = `cbz w0`、`0x48a0b44` = `str xzr`，与 fzlzjerry `revoke-tip` 对 269110 的补丁逐字节同构。）
